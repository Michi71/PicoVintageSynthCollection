...
| [`d5_midi/`](d5_midi/README.md) | D5 | a five-minute MIDI file that tours the patch bank -- it selects patches over CC 0 and program change and plays each one a phrase that suits it, chosen to show what is peculiar about the machine. Plus the generator that produced it. |
...
| [`d5_extract/`](d5_extract/README.md) | D5 | D-50 ROM identification and PCM decoding, the reconstructed sample table, the firmware blob generator, the SysEx patch-bank converter, the D-05 multi-bank extractor, and a host harness that renders the engine to WAV. Needs a ROM set, which is not in this repository. |
