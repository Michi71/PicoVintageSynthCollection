...
enum {
    IPC_CMD_J6_NOTE_ON    = 0x0B,
    IPC_CMD_J6_NOTE_OFF   = 0x0C,
    IPC_CMD_J6_PARAM      = 0x0D,
    IPC_CMD_J6_PITCH_BEND = 0x0E,
    IPC_CMD_J6_CC         = 0x0F
};
...
inline volatile uint32_t j6_ipc_buf[256];
inline volatile uint16_t j6_ipc_head = 0;
inline volatile uint16_t j6_ipc_tail = 0;
inline volatile uint32_t j6_ipc_dropped = 0;  // diagnostic (shown as D on the OLED)

static inline void j6_ipc_push(uint32_t pkt) {
    uint16_t next_head = (j6_ipc_head + 1) & 255;
    if (next_head != j6_ipc_tail) {
        j6_ipc_buf[j6_ipc_head] = pkt;
        j6_ipc_head = next_head;
    } else {
        j6_ipc_dropped++;
    }
}

static inline bool j6_ipc_pop(uint32_t* pkt) {
    if (j6_ipc_head != j6_ipc_tail) {
        *pkt = j6_ipc_buf[j6_ipc_tail];
        j6_ipc_tail = (j6_ipc_tail + 1) & 255;
        return true;
    }
    return false;
}
...
static inline void ipc_send_note_on(uint8_t note, uint8_t vel) {
    j6_ipc_push(ipc_pack(IPC_CMD_J6_NOTE_ON, note, vel));
}

static inline void ipc_send_note_off(uint8_t note) {
    j6_ipc_push(ipc_pack(IPC_CMD_J6_NOTE_OFF, note, 0));
}

static inline void ipc_send_cc(uint8_t cc, uint8_t value) {
    j6_ipc_push(ipc_pack(IPC_CMD_J6_CC, cc, value));
}

static inline void ipc_send_param(uint8_t paramId, uint16_t value) {
    j6_ipc_push(ipc_pack(IPC_CMD_J6_PARAM, paramId, value));
}

static inline void ipc_send_pitch_bend(uint16_t bend14) {
    j6_ipc_push(ipc_pack(IPC_CMD_J6_PITCH_BEND, 0, bend14));
}
