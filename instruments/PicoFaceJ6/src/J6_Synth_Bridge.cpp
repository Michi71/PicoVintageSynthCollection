...
void J6_Synth_Bridge::init()
{
    juno_.setSampleRate((float) SAMPLING_RATE);
    juno_.setProgram(0);   /* Fat Bass */
}
 ...
 *
 * The Juno-60 is mono; both channels carry the same signal.
 */
void RAM_HOT(J6_Synth_Bridge::fill_buffer_i32)(int32_t* out, int length)
{
    ...
    while (done < length)
    {
        ...
        for (int i = 0; i < chunk; ++i)
        {
             ...
             * all" symptom on an earlier project in this family: the
             * output measured -111.5 dBFS instead of -15.2.
             *
