...
/*
  MD_Synth_Bridge.cpp -- wiring the Moog engine to the audio subsystem
*/
...
void MD_Synth_Bridge::init()
{
    moog_.setSampleRate((float) SAMPLING_RATE);
    moog_.setProgram(0);   /* Fat Bass */
}

/*
 * The I2S buffer expects interleaved stereo int32. The engine delivers float
 * and its output stage already limits softly, so scaling is all that is
 * needed here -- a second limiter would only distort twice.
 *
 * The Model D is mono; both channels carry the same signal.
 */
void RAM_HOT(MD_Synth_Bridge::fill_buffer_i32)(int32_t* out, int length)
{
    ...
    while (done < length)
    {
        ...
        for (int i = 0; i < chunk; ++i)
        {
             ...
             * all" symptom on the Solina build: the output measured
             * -111.5 dBFS instead of -15.2.
             *
