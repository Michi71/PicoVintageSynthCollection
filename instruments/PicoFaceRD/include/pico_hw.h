...
// without it, decaying IIR states settle into the subnormal range where the
// Cortex-M33 FPU falls back to a much slower software path (audible as
// intermittent hiss under load). Flushing denormals is inaudible (values are
// already below the noise floor) and standard practice for real-time DSP.
