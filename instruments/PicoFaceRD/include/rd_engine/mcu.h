...
extern volatile uint32_t g_rd_prof_chip_us;  // accumulated us in sound_chip.update()
extern volatile uint32_t g_rd_prof_cpu_us;   // accumulated us in 6301 interpreter loop
#endif

// Fixed ring buffer for MIDI command framing in the 6301 emulation.
// No heap allocation in IRQ context. Commands are 1 or 3 bytes.
// Partial enqueues corrupt the 6301 command framing, hence push3
// provides whole-command semantics: either all three bytes are
// enqueued atomically or none at all.
struct RdCmdQueue {
    static constexpr uint16_t CAP = 1024;

    u8 buf_[CAP];
    uint16_t head_ = 0;
    uint16_t tail_ = 0;
    volatile uint32_t dropped_ = 0; // diagnostic: whole commands dropped
    ...
    void pop() {
        if (head_ != tail_) head_ = (head_ + 1) % CAP;
    }

    size_t freeSpace() const {
        return CAP - 1 - ((tail_ - head_ + CAP) % CAP);
    }
...
public:
  ...
  void execute_run(int cycles);  // run the CPU for a cycle budget (cycle-accurate, ~2 MHz real speed)

  RdCmdQueue commands_queue;

	s32 generate_next_sample(bool sampleRate32 = false);
	bool current_sample_rate = false;

	void sendMidiCmd(u8 cmd, u8 data1, u8 data2);
	void loadSounds(const u8 temp_model, size_t from_addr);

	void loadPatch(const u8 temp_patch);
	s32 get_next_sample(void);
	u8 getPatch(void);
	void getPatchName(char *patchName);
	u32 getCurrentSampleRate() { return current_smple_rate; }
	SoundChip* getSoundChip() { return &sound_chip; }  // for dual-core worker_enable
	void reset();
...
private:
  ...
  u8 latch_val = 0x00;
  u8 current_patch = 0x00;
  u32 current_smple_rate = 20000;
  ...
  static u8 flags8i[256];  // RAM: read every emulated instruction
	static u8 flags8d[256];
  enum
	{
  ...
  static u8 cycles_63701[256];
  
  static op_func hd63701_insn[256];  // RAM: opcode dispatch, hottest table
  void aba();
	void abx();
	void adca_di();
	void adca_ex();
