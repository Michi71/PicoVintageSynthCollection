...
class Voice
{
  ...
  public:
    ...
    /* Base values in native units, set by the engine's parameter setters. */
    ...
    inline float __not_in_flash_func(ProcessSample)()
    {
        ...
        // tuned note.
        const float tunedNote = (float)midiNote;
        ...
        // voice, to apply matrix offsets without accumulating them - roughly
        // 26 loads and stores per voice and sample that this port does not
        // spend.
    ...
    void setBrightness(float val)
    {
        ...
        state.brightnessCoef =
            ob_tan(std::min(par.osc.brightness, (sampleRate * 0.5f) - 10) * pi * sampleRateInv);
    }
