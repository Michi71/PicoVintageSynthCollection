 ...
 */
/*
 * PORTED FOR PicoFaceOB (RP2350): 16 unsuffixed floating point literals
 * got an f suffix. On a host they cost nothing; here every one of them
 * promoted its whole expression to software double precision, in the
 * per-sample path.
 */
/*
...
class TriangleOsc
{
  ...
  public:
    ...
    inline void processLeader(float x, float delta)
    {
        ...
        {
            mixInBlampCenter(buffer, bufferPos, (x - 0.5f) / delta, 4 * B_SAMPLES * delta);
        }
    ...
    inline void processFollower(float x, float delta, bool hardSyncReset, float hardSyncFrac)
    {
        ...
        if (hardSyncReset)
        {
            ...
            mixInImpulseCenter(buffer, bufferPos, hardSyncFrac, mix + 0.5f);
        }
    }
