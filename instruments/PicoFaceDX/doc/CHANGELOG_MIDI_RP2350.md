...
> **Archived / superseded:** this changelog describes the reface CP MIDI layer,
> which was removed entirely when this project became DX-only. The MIDI
> specification that actually applies (reface DX) is in
> [`MIDI_IMPLEMENTATION.md`](MIDI_IMPLEMENTATION.md). This document is kept as
> historical context for why `RefaceMidi` was built the way it was.

> **Historical document.** It also predates the move into the collection: the
> core0/core1 split it describes is gone, everything runs on core0 now. See
