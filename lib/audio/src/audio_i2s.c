...
// Diagnostic: counts DMA underruns (silence buffer substituted because no
// filled producer buffer was available). Read-only for UI display.
volatile uint32_t g_i2s_underrun_count = 0;

// Diagnostic: PIO TX-FIFO stall detector. The sticky FDEBUG.TXSTALL bit is set
// by hardware whenever the state machine starved (audible click that the
// silence-buffer counter above can NOT see, e.g. while the DMA IRQ handler is
// still busy rendering). Call from the producer, returns 1 if a stall happened
// since the last call and clears the flag.
uint32_t audio_i2s_consume_txstall(void) {
    uint32_t bit = 1u << (PIO_FDEBUG_TXSTALL_LSB + shared_state.pio_sm);
    if (audio_pio->fdebug & bit) {
        audio_pio->fdebug = bit; // write-1-to-clear
        return 1u;
    }
    return 0u;
}
...
static void update_pio_frequency(uint32_t sample_freq, audio_pcm_format_t pcm_format, audio_channel_t channel_count) {
    // NO printf here: this runs inside the DMA IRQ on a sample-rate switch; a
    // blocking 115200-baud UART line stalls the PIO for 15-20 ms (audible).
