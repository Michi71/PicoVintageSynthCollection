...
// Diagnostic underrun counter (see audio_i2s.c): incremented whenever the DMA
// had to substitute the silence buffer.
extern volatile uint32_t g_i2s_underrun_count;

// Returns 1 and clears the flag if the PIO TX FIFO stalled (starved) since the
// last call -- catches underruns invisible to g_i2s_underrun_count.
